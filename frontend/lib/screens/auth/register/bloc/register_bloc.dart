...
class RegisterBloc extends Bloc<RegisterEvent, RegisterState> {
  ...
  void _onEmailChanged(EmailChanged event, Emitter<RegisterState> emit) {
    emit(RegisterEdited(email: event.email, password: state.password, confirmPassword: state.confirmPassword));
  }

  void _onPasswordChanged(PasswordChanged event, Emitter<RegisterState> emit) {
    emit(RegisterEdited(email: state.email, password: event.password, confirmPassword: state.confirmPassword));
  }

  void _onConfirmPasswordChanged(ConfirmPasswordChanged event, Emitter<RegisterState> emit) {
    emit(RegisterEdited(email: state.email, password: state.password, confirmPassword: event.confirmPassword));
  }
